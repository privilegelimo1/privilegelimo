...
type FleetItem = {
  id: string;
  slug: string;
  class_slug: string;       // ← actual column name
  name: string;
  category: string;         // ← "Executive Saloon / Sedan"
  passengers: number;
  luggage: number;
  ...
  price_10hr?: string;
  images?: string[];
  short_desc?: string;      // ← actual column name
  features?: string[];
  badge?: string | null;
  is_featured?: boolean;
  sort_order?: number;
...
export default function FleetGrid({ fleet, categories }: Props) {
  ...
  // Only show categories that have at least one vehicle
  const availableClasses = useMemo(() => {
    return categories.filter((cat) =>
      fleet.some((v) => v.class_slug === cat.slug)   // ← class_slug === cat.slug
    );
  }, [fleet, categories]);
  ...
  const grouped = useMemo(() => {
    return availableClasses.reduce<Record<string, FleetItem[]>>((acc, cat) => {
      acc[cat.slug] = fleet.filter((v) => v.class_slug === cat.slug);  // ← key by slug
      return acc;
    }, {});
  ...
  const handleSelect = (slug: string) => { setActiveClass(slug); setOpen(false); };
      ...
      <section className="py-28 bg-gradient-to-b from-[#AB5461]/3 to-[#ab5461]/10">
        <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="flex flex-wrap justify-center gap-8 text-center mb-8">
                  {[
                    ...
                    <div key={label}>
                      ...
                      <p className="text-xs text-[#8c8c8c] uppercase tracking-[0.22em] mt-1">{label}</p>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="sm:ml-auto flex items-center gap-3">
                    ...
                    <div className="relative">
                      ...
                      >
                        <span>
                          {activeClass
                            ? availableClasses.find((c) => c.slug === activeClass)?.display_name
                            : "All Fleet Classes"}
                        </span>
                        <ChevronDown
                      ...
                      {open && (
                        <div className="absolute top-full right-0 mt-2 w-72 bg-white rounded-2xl border border-[#f1dfe2] shadow-[0_18px_50px_rgba(0,0,0,0.12)] overflow-hidden z-50">
                          <button
                            onClick={handleClear}
                            className={`w-full text-left px-4 py-3 text-sm flex items-center justify-between gap-2 transition-all ${!activeClass ? "font-semibold text-white" : "font-medium text-[#5f5f5f] hover:bg-[#fcf6f7]"}`}
                            style={!activeClass ? { background: "linear-gradient(135deg, #AB5461 0%, #c67b86 100%)" } : {}}
                          >
                            <span className="flex items-center gap-2">
                              All Fleet Classes <span className="text-xs opacity-70">({fleet.length})</span>
                            </span>
                            {!activeClass && <Check className="text-white shrink-0" />}
                          </button>
                          ...
                          {availableClasses.map((cls, i) => (
                            <button
                              ...
                              className={`w-full text-left px-4 py-3 text-sm flex items-center justify-between gap-2 transition-all ${
                                activeClass === cls.slug ? "font-semibold text-white" : "font-medium text-[#5f5f5f] hover:bg-[#fcf6f7]"
                              } ${i !== 0 ? "border-t border-[#faf0f2]" : ""}`}
                              style={activeClass === cls.slug ? { background: "linear-gradient(135deg, #AB5461 0%, #c67b86 100%)" } : {}}
                            >
                              <span className="flex items-center gap-2">
                                {cls.display_name}
                                <span className="text-xs opacity-70">({grouped[cls.slug]?.length ?? 0})</span>
                              </span>
                              {activeClass === cls.slug && <Check className="text-white shrink-0" />}
                            </button>
                          ))}
                        </div>
                      )}
          ...
          {visibleCategories.map((cat) => {
            const vehicles = grouped[cat.slug] ?? [];   // ← key by slug
              ...
              <div key={cat.slug} className="mb-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {vehicles.map((v) => {
                    ...
                    const shortDesc = (v.short_desc ?? "").substring(0, 85);  // ← short_desc
                      ...
                      >
                        ...
                        <div className="p-7">
                          ...
                          <div className="flex items-center justify-between mb-4">
                            <Link
                              href={`/fleet/${v.class_slug}/${v.slug}`}
                          ...
                          <BookingButton
                            ...
                            transferPrice={v.transfer_price ?? "Contact for Rate"}
                            price5hr={v.price_5hr ?? "Contact for Rate"}
                            price10hr={v.price_10hr ?? "Contact for Rate"}
