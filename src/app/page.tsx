      ...
      <motion.div style={{ opacity }} className="relative z-10 max-w-5xl mx-auto px-6 text-center">
        ...
        >
          ...
          <span className="text-[#c9a84c] text-[10px] tracking-[0.6em] uppercase font-light">
            25 Years · Premium Chauffeur Service · Dubai UAE
          </span>
          <div className="h-px w-10 bg-[#c9a84c]" />
        </motion.div>
            ...
            {fleet.map((car, i) => (
              <AnimateIn key={car.id} delay={i * 0.06} direction="up">
                <article className="group bg-white rounded-[28px] border border-[#efefef] hover:border-[#0a0a0a] hover:shadow-[0_20px_60px_rgba(0,0,0,0.08)] transition-all duration-500 overflow-hidden h-full flex flex-col">
                  {/* Image */}
                  <div className="relative h-52 bg-[#fafafa] border-b border-[#f5f5f5] flex items-center justify-center overflow-hidden">
                    {car.badge && (
                      <div className="absolute top-4 left-4 z-10">
                        <span className="text-[9px] tracking-[0.3em] uppercase bg-[#0a0a0a] text-white px-3 py-1.5 rounded-full">
                          {car.badge}
                        </span>
                      </div>
                    )}
                    <div className="absolute top-4 right-4 z-10">
                      <span className="text-sm font-light text-[#0a0a0a] bg-white border border-[#efefef] px-4 py-1.5 rounded-full shadow-sm">
                        {car.priceLabel}
                      </span>
                    </div>
